...
from vision_kit.utils.image_proc import resize
from vision_kit.utils.bboxes import xywh_to_xyxy, xywhn_to_xyxy, xyxy_to_xywh, xyxy_to_xywhn
...
class TrainAugPipeline:
    def __init__(
        ...
        self.img_sz = (img_sz, img_sz) if isinstance(img_sz, int) else img_sz
    ...
    def __call__(self, img, labels):
        try:
            ...
            transformed_img, (ratio, pad) = resize(self.img_sz, img)
            ...
            if self.bbox_format == "yolo":
                ...
                labels[:, [1, 3]] += pad[1]
                labels[:, :4] *= ratio
                labels = xyxy_to_xywhn(
                    labels, w=transformed_img.shape[1], h=transformed_img.shape[0])
