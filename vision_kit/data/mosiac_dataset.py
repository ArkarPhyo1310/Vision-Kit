...
class MosaicDataset(Dataset):
    ...
    def __init__(
        ...
        mosaic_prob=1.0, mixup_prob=1.0, label_format="coco", *args
    ):
        """
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            mosaic_img, mosaic_labels, img_id = self.mosiac(
                idx, self._dataset.input_dim)
        ...
        if self.label_format == "coco":
            ...
            label = xyxy_to_xywhn(
                label, w=img.shape[1], h=img.shape[0])
    ...
    def mosiac(self, idx, input_dim):
        ...
        indices = [
            idx] + [random.randint(0, len(self._dataset) - 1) for _ in range(3)]
